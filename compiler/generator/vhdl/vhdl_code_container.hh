...
struct VhdlValue {
    ...
    VhdlValue(double value): value(), vhdl_type(gGlobal->gVHDLFloatEncoding ? VhdlType(VhdlInnerType::RealFloat, 8, -23) : VhdlType(VhdlInnerType::SFixed, 8, -23)) { this->value.real = value; }
...
class VhdlCodeContainer
{
   ...
   public:
    ...
    // Connects two nodes generateRegisterSeries(with the given amount of lag i.e registers in between source and target
     ...
     */
    size_t generateRegisterSeries(int n, VhdlType type, std::optional<size_t> source = std::nullopt);
    ...
    void generateSineOperator(size_t hash, VhdlType type);
